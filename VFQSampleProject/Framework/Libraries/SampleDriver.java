...
public class SampleDriver {
	...
	public static ThreadLocal<String> ServiceIDSeibel = new ThreadLocal<String>();
	public static ThreadLocal<String> ApplicationName = new ThreadLocal<String>();
	public static ThreadLocal<String> Execution_Environment = new ThreadLocal<String>();	
	...
	public static ThreadLocal<String> Promotion_Name = new ThreadLocal<String>();
	public static ThreadLocal<String> RTNum = new ThreadLocal<String>();
	public static ThreadLocal<String> alertMsg1 = new ThreadLocal<String>();
	public static ThreadLocal<String> ActivityRef1 = new ThreadLocal<String>();
	public static ThreadLocal<String> ActivityRef2 = new ThreadLocal<String>();
	public static ThreadLocal<String> Auth_Login = new ThreadLocal<String>();
	public static ThreadLocal<String> LastName = new ThreadLocal<String>();
	public static ThreadLocal<String> ID_No = new ThreadLocal<String>();
	...
	public static void ExecuteCases(String Batch, String batchcount) {
		...
		Dunning.set(Temp_FLD.get() + "/" + Batch + "/DunningCalendar.xlsx");//added by sisira
		TestDataDB_File.set(Temp_FLD.get() + "/" + Batch + "/TestDataDB.xlsx");
		...
		try {
				...
				for (int currKeyword = 0; currKeyword < totKeywords.length; currKeyword++) {
					...
					if (Continue.get() == true) {
						...
						if (currKW_DB.get().toString().equalsIgnoreCase("Data")) {
							...
							Execution_Environment.set(getdata("Execution_Env"));
			...
			/*
			 * if (TotExeCount == totalUCount) { // Result.DisplayHTMLReport();
			 * Browser.OpenBrowser("chrome", masterrephtml.get()); killexeTask(); }
			 */
			if (TotExeCount == totalUCount) {
                // Result.DisplayHTMLReport();
                browser.set("Chrome");          //COMMENTED FOR LAMBDA TEST 
//                Browser.OpenBrowser(browser.get(), masterrephtml.get());   // COMMENTED FOR LAMBDA TEST 
                killexeTask();
            }
		} catch (Exception e) {
			e.printStackTrace();
...
class RunnableDemo implements Runnable {
	...
	public void run() {
		...
		try {
			SampleDriver.ExecuteCases(threadName, batchcount);
		} catch (Exception e) {
